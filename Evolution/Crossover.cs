...
namespace Brain.Evolution
{
	public static class Crossover
	{
		...
		{
			var p1 = Util.RandomInt(1, left.Length);
			var p2 = Util.RandomInt(1, right.Length);
		...
		{
			var idxs = Util.RandomUniqueInts(2, 0, left.Length);
			Array.Sort(idxs);
		...
		{
			var swapLength = Util.RandomInt(1, left.Length - 1);
			var swapIndexes = Util.RandomUniqueInts(swapLength, 0, left.Length);
		...
		private static Chromosome CreatePositionBasedOffspring(Chromosome left, Chromosome right,
			int[] swapIndexes)
		{
		...
		private static Chromosome CreateTwoPointOffspring(Chromosome left, Chromosome right, int point1,
			int point2)
		{
		...
		{
			var cutPointsIndexes = Util.RandomUniqueInts(2, 0, left.Length);
		...
		private static Gene GetGeneNotInMappingSection(Gene candidateGene, Gene[] mappingSection,
			Gene[] otherMappingSection)
		{
			var idx = -1;
